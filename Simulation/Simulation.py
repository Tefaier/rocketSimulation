...
earthRotation = Rotation.from_rotvec(23.5 * vecNormalize(earthPosition), degrees=True) * Rotation.align_vectors(np.array([0, 0, 1]), earthAxis)[0]
...
marsRotation = Rotation.from_rotvec(25.2 * vecNormalize(earthPosition), degrees=True) * Rotation.align_vectors(np.array([0, 0, 1]), earthAxis)[0]
...
rocketRotation = Rotation.from_quat(quaternion) * earthRotation
rocketPosition = earthPosition + (rocketRotation).apply(np.array([0, 0, 1])) * earthRadius
...
mksMass = 440000
mksFlyHeight = 418000
mksRotationProjected = Rotation.from_quat(quaternion) * earthRotation
mksVelocity = mksRotationProjected.apply(np.array([0, 1, 0])) * 7700
mksPosition = earthPosition + (mksRotationProjected).apply(np.array([0, 0, 1])) * (earthRadius + mksFlyHeight)
...
def executeFrame(frameTime: pd.Timedelta, entities: List[SimulationEntity]):
    ...
    def applyChanges():
        for obj in entities:
            ...
            obj.velocity += acceleration * frameTime
            obj.position += obj.velocity * frameTime
